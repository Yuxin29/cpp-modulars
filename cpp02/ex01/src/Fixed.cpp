...
--> toInt: 10859 >> 8 = 10859 / 2^8  = roundf(10859 / 256)                         // not need to round: al ready rounded here --->|
...
Fixed::Fixed(const int value){
    ...
    if (value > (MY_INT_MAX >> _fractionalBitsNumber))
        _fixedPointValue = MY_INT_MAX;
    else if (value < (MY_INT_MIN >> _fractionalBitsNumber))
        _fixedPointValue = MY_INT_MIN;
    else
